...
*Streams* is built on native Node.js streams.  Node represents sockets as streams; hence, it's natural and easy to connect *Streams* logging graphs over the network.  Please see the [*A Network Connected **Streams** Logging Graph*](#a-network-connected-streams-logging-graph-example) example for instructions on how to use a `SocketHandler` in order to construct network connected *Streams* logging graphs.
